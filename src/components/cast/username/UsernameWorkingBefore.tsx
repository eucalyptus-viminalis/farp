...
// Assumming root cast
export default function UsernameWorkingBefore(props: UsernameProps) {
    // Props
    const { asEmbed } = props;
    // Context
    ...
    const overrideUsername = (username: string) => {
        context.dispatch({type:'SET_ROOT_CAST',payload:{...cast, usernameOverride: username}})
    }
    ...
    return (
        <>
            <label 
                htmlFor="user-search-input" ref={containerRef}
                hidden={!inputFocused}
            >
                <input
                    ref={inputRef}
                ...
                "
                    ...
                    // onSelectCapture={()=>alert('select captured')}
                />
                <ul className="w-96 absolute z-10 rounded-lg border border-faint bg-app overflow-auto max-h-72">
                    {showUsers &&
    ...
    if (inputFocused) {
        return (
            ...
            //     onBlur={handleBlur}
            // />
            <UserSearchInput hidden={!inputFocused} toggleView={toggleInput} />
        );
    } else {
        return (
